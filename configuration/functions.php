...
function getCompleteUserDetail()
{
	...
	$grp = array($jan, $feb, $mar, $apr, $mar, $jun, $jul, $aug, $sep, $oct, $nov, $dec);
	$arr = array();
	$arr[0] = $total;
	$arr[1] = $draft;
	$arr[2] = $complete;
...
{
	global $con;
	$execQry = mysqli_fetch_array(mysqli_query($con, "select * from `package` where `id`='$id'  "));
	return $execQry;
}
...
{
	if ($value == 1) {
		return '<span class="badge badge-pill badge-info">Upload</span>';
	}
	if ($value == 2) {
		return '<span class="badge badge-pill badge-warning">App</span>';
	}
	if ($value == 3) {
		return '<span class="badge badge-pill badge-primary">Browser</span>';
	}
	if ($value == 4) {
		return '<span class="badge badge-pill badge-danger">Blog</span>';
	}
	if ($value == 5) {
		return '<span class="badge badge-pill badge-success">Script</span>';
	}
}
...
function sendForgotPasswordMail($to, $from, $fromname, $subject, $msg)
{
	// $headers = 'MIME-Version: 1.0 \r\n'.
	// 	'Content-type: text/html \r\n'.
	// 	'X-Mailer: PHP/' . phpversion();
	...
	// $header = "From:rsrsanjeev@gmail.com \r\n";
	// $header .= "Cc:rsrsanjeev@hotmail.com \r\n";
