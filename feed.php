...
//$qry=mysqli_query($con,"SELECT * FROM `chapter` where id=9");
...
if (isset($_GET['cpid']) && $_GET['cpid'] != '') {
  ...
  $fetch = mysqli_fetch_array($qry);
  $link = $baseurl . "feed.php?cid=" . $_GET['cid'];
}
?>
<!doctype html>
<html lang="en">
...
<body style="background-color:<?= $fetch['color'] ?>">
  <div class="giide-outer-container" style="background-color:<?= $fetch['color'] ?>">
    <div class="outer-gradient" style="background-color:<?= $fetch['color'] ?>">
      <a href="#" class="logo"><img src="./assets/img/logo.png" alt="" style="width: 40%;"></a>
      <input type="hidden" name="hidChapId" id="hidChapId" value="<?php echo $cid ?>">
      <input type="hidden" name="ownerId" id="ownerId" value="<?php echo $fetch['user_id']; ?>">
    ...
    <div class="giide-aspect-ratio" style="width: 371.25px; height: 660px; margin-top: 10px;">
      <div class="giide-view-content">
          ...
          <div class="share-modal" style=" width: 315.562px; height: 165px;">
            <div>
              <div class="d-flex" style="justify-content: space-between;">
                <span>
                  <h1 style="font-size: 18.5625px;width:200px">Share this feeds</h1>
                </span>
            ...
            <div class="mt-3 text-center">
              <a target="_blank" href="<?= $fetch['cta'] ?>" class="btn btn-dark">CTA Button</a>
            </div>
          </div>
        </div>
          ...
          <div style="width: 371.25px;">
            ...
            <div id="div1" class="giide-feed-container" id="giide-feed-container" style="height: 554.4px;">
              <div class="inner-flex-container">
                    ...
                    <div class="feed-card-container" style="width: 334.125px; margin: 0px 11.1375px 14.85px;">
                      <div class="title-card feed-card" style="height: 513.228px;">
                        <div class="title-image-container" style="width: 337.34px; height: 337.34px;">
                          <img src="<?= $fetch['image'] ?>" alt="<?= $fetch['name'] ?>" style="display: block;">
                ...
                ?>
                <?php $i = 1;
                while ($urlFetch = mysqli_fetch_array($urlQry)) {
                  $i++; ?>
                ...
                <?php } ?>
                <?php $i = 1;
                while ($quoteFetch = mysqli_fetch_array($quoteQry)) {
                  $i++; ?>
                ...
                <?php } ?>
                <?php $i = 0;
                while ($highFetch = mysqli_fetch_array($highlightQry)) {
                  $i++; ?>
                ...
                <?php } ?>
                <?php $df = 0;
                while ($defFetch = mysqli_fetch_array($definationQry)) {
                  $df++; ?>
                ...
                <?php } ?>
                <?php $i = 1;
                while ($imgFetch = mysqli_fetch_array($imageQry)) {
                  $i++; ?>
                ...
                <?php } ?>
                <?php $i = 1;
                while ($profileFetch = mysqli_fetch_array($profielQry)) {
                  $i++; ?>
                  ...
                  <div style="display:none" id="pp_<?= $i ?>" name="b129d5ac-fbb8-4aba-b3cb-5128c836bcaf" class="giide-feed-child" style="padding: 3.7125px;border-radius:4px;background:#fff">
                    <div id="contentDiv" class="feed-card-container" style="width: 334.125px; margin: 0px 11.1375px 14.85px;">
                      <div class="border p-2  mb-2 bg-white">
                        <div class="flex-shrink-0">
                          <img alt="Image placeholder" class="avatar rounded-circle" src="<?= $profileFetch['image'] ?>">
                        </div>
                        <div class="flex-grow-1 ms-3">
            ...
            <div class="player-container" style="height: 105.6px;">
              <div style="width: 0px; height: 0px;">
                <audio style="width: 100%; height: 100%;" id="audio" loop="false" class="w-100" controls>
                  <source src="audio/<?= $fetch['audio_file'] ?>" type="audio/mp3">
                </audio>
              ...
              <div class="player-buttons" style="height: 72.6px;">
                ...
                <button id="back" type="button" class="rewind-button" style="width:0%; height: 37.125px;"><i class="fa-solid fa-backward-step"></i></button>
                <button type="button" class="play-button" style="width: 51%; height: 57.125px;" onClick="togglePlay()"><i id="aplay" class="fas fa-play"></i><i id="apause" style="display:none" class="fas fa-pause"></i></button>
                <button id="fwd" type="button" class="rewind-button1" style="width:24%; height: 37.125px;"><i class="fa-solid fa-forward-step"></i></button>
                <div type="button" class="playback-rate-button dropup" style="width: 29.7px;    "><img src='assets/img/feed_image/reactions.png' style="width:100%" class="dropdown-toggle " id="dropupMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                ...
                <?php if ($fetch['brand'] == '0') { ?>
                  <a href="#" target="_blank" rel="noopener noreferrer" class="player-logo-link" style="width: 44.55px; height: 37.125px;">Giide</a>
                <?php } else {
        ...
        <div class="control">
          <div onClick="togglePlay()" class="button" id="play-pause-button">
          <i id="aplay" class="fas fa-play"></i><i id="apause" style="display:none" class="fas fa-pause"></i>
          </div>
        </div>
        <div class="control">
          <div class="button rewind-button1"  id="fwd">
            <i class="fas fa-forward"></i>
          </div>
        </div>
        <div class="control">
          <div type="button" class="button playback-rate-button dropup" ><img src='assets/img/feed_image/reactions.png' style="width:100%" class="dropdown-toggle " id="dropupMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                  <div class="dropdown dropup">
                    <div class="dropdown-menu" aria-labelledby="dropupMenuButton">
        ...
        <div class="control">
          <button id="shareBtn" type="button" class="button open-menu-button" style="height: 37.125px;"><img src="data:image/png;base64," alt="open giide menu"></button>
                <?php if ($fetch['brand'] == '0') { ?>
                  <a href="#" target="_blank" rel="noopener noreferrer" class="player-logo-link" style="height: 37.125px;">Giide</a>
                <?php } else {
                  $brand = getFeedBrand($fetch['brand']);
                ?>
  ...
  <script>
    ...
    function addReaction(id) {
      ...
      var oid = document.getElementById('ownerId').value; //owner id 
      $.ajax({
        type: 'POST',
        dataType: 'json',
        url: 'addReaction.php',
        ...
        error: function(XMLHttpRequest, textStatus, errorThrown) {
          ...
          alert(textStatus + " Status: Audio format not supported kindly create you own audio" + errorThrown);
        ...
        success: function(response) {
          if (response.message == "success") {
              ...
              alert(`error ${response.message} ${response.info}`);
            }
        }
      });
      // $.ajax({
  ...
  <script>
    ...
    var advance = function(duration, element) {
      ...
      //    var sec =(element.currentTime%60);
      //    var min =(element.currentTime/60);
      var total = min + ":" + sec;
